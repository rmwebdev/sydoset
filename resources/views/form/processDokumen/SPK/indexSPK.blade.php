            ...
            <div class="row">
                <div class="col-lg-12">
                    <div class="card-body" style="min-height:90vh">
                        <div class="row">
                            <div class="col-lg-4 mr-sm-2" style="">
                                <h4 class="box-title">REGISTRASI NOMOR SPK FINANCE</h4>
                            </div>
    ...
    <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
      <div class="modal-content ">
        <div class="modal-header">
          <label class="modal-title" id="regMdl" style="font-size:18px;">TAMBAHKAN DOKUMEN BARU</label>
    ...
    (function($) {
        $(document).ready(function() {
            ...
            table = $('#example').DataTable({
                        ajax: {
                            url: '/getDatatableProccessDocSPK',
                            type:"POST",
                            dataSrc:'',
                        data : {
                            type:1
                    ...
                    "initComplete": function(settings, json) {
                        ...
                        {   
                                ...
                                render: function(data, type, full, meta)
                                {
                                    ...
                                    return '<a href="insertProccessDocSPK/'+$encoded+'" style="text-decoration: underline;">'+full.number+'</a>';
                                    
                                }
                            },
                        { data:'order_number', title:'ORDER NUMBER', className: 'dt-td-center'},
                        { data:'tender_time', title:'TENDER TIME', className: 'dt-td-center', 
                            render: function(data, type, full, meta){
                                return moment(data).format('DD MMM YYYY');
                            }
                        },
                        { data:'updated_by', title:'LAST USER', className: 'dt-td-center'},
                        { data:'last_status', title:'STATUS', className: 'dt-td-left',
                            render: function(data, type, full, meta){
                                    if(data == 'registration_doc'){
                                        return 'Diterima Admin Settle';
                                    }
                                    else {
                        ...
                        { data: 'catatan', title:'REMARKS', className: 'dt-td-center',
                            render: function(data, type, full, meta)
                            {
                                if (data != null) {
                    ...
                    table.on( 'draw', function () {
                        ...
                        var buttonOpenDoc = '<div class="clearfix" style="float:right"><button class="btn" style="display:none;background:transparent !important;color:#0d66fa;margin-right:8px" onClick="hideCheck()">BATAL</button><button class="btn btn-info" style="" onClick="openModal(\'kembalikan\')">Kembalikan Dokumen</button><button class="btn btn-info" style="display:none;" data-toggle="modal" data-target="#modal_openDoc" onClick="clearData()">Open Document</button></div>';
                ...
                tabelRegDoc = $('#tabelRegDoc').DataTable({
                    ...
                    columns: [ 
                        ...
                        {  className: 'dt-td-right', title:'TENDER TIME'},
                        {  className: 'dt-td-left', title:'LAST USER'},
                        {  className: 'dt-td-left', title:'STATUS'},
                        {  className: 'dt-td-left', title:'REMARKS'}
                    ],
                    "columnDefs": [
                ...
                $('#searchRegDoc').keyup(function(e){
                    if(e.keyCode == 13)
                    {
                        $.ajax({
                            method: "POST",
                            url: "{{route('regDocProcess')}}",
                            data: { 
                                barcode : $('#searchRegDoc').val(),
                                barcodeSearch:$('#searchRegDoc').val(),
                                type : 1
                            },
                            success: function (data) { 
                                if(data.stat == 1){
                                    ...
                                    var input = document.getElementById('searchRegDoc');
                                    input.focus();
                                    input.select();
